...
At an interim analysis, the default decisions from OCTOPUS are utilized see help with with ?MakeDecisionBasedOnPostProb.MAVOnly.  In particular, if $p_{MAV} >$ `r lTrialDesigns[[1]]$cISADesigns$cISA1$cISAAnalysis$vAnalysis[[1]]$vPUpper[1] ` the a Go is declared and the ISA is stopped early;  If $p_{MAV} <$  `r lTrialDesigns[[1]]$cISADesigns$cISA1$cISAAnalysis$vAnalysis[[1]]$vPLower[1] ` then a No Go is declared an the ISA stopped early; otherwise an indeterminate decision is made and the trial continues.  
...
PlotResults( dfResSub, vScenarioLabel = vScenarioLabel)
