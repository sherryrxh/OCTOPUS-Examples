...
#
#   This example includes 3 ISAs and we use a monitoring schedule that runs the first IA for an ISA
#   when half the patients have 6 months FU then performs the analysis monthly thereafter.  See TrialDesign.R
#   Interim Analysis - Example 2 to see how the monitoring is setup.
#
#   In this same file we have updated the prior for control to be a Beta( 10, 40 )
#
...
#       In this file the default ISA start times (when the ISA enters the platform) have been setup to be random
#
#
...
dQtyMonthsFU       <- 6
mQtyPatientsPerArm <- matrix( c( 50,25,25,100,125,125 ), nrow=3, ncol = 2 )
...
# This design has an interim analysis for each ISA when half the patients are enroll with 6 months FU and a final analysis
...
mMinQtyPats       <- cbind( floor(apply( mQtyPatientsPerArm , 1, sum )/2),  apply( 2*mQtyPatientsPerArm , 1, sum ) )
vMinFUTime        <- rep( dQtyMonthsFU, ncol( mMinQtyPats) )
dQtyMonthsBtwIA   <- 0
...
# then a Go decision is reached, if it is less than 0.01 a No Go decision is reached, otherwise the trial continues to the next analysis.
...
# This design has an interim analysis for each ISA when half the patients are enroll with 6 months FU and has analysis monthly
# thereafter because dQtyMonthsBtwIA   <- 1
